...
bos_start() {
  ...
  bos_atomic_json "$BOS_STATE" "$(jq -n --arg profile "$profile" --arg runtime "$(bos_profile_value "$profile" runtime)" --arg platform "$BOS_PLATFORM" --arg started "$(date -u +%FT%TZ)" '{profile:$profile,runtime:$runtime,platform:$platform,started_at:$started}')"

  bos_info "Starting $profile..."
  local attempt timeout="${BOS_START_TIMEOUT:-1800}"
...
bos_status() {
  ...
  local rss="-" cpu="-" uptime="-"
  if [[ -n "$pid" ]]; then
    rss="$(ps -p "$pid" -o rss= 2>/dev/null | tr -d ' ' || true)"
    [[ -n "$rss" ]] && rss="$(bos_format_bytes "$((rss * 1024))")"
    rss="${rss:--}"
  ...
  printf "%-18s %s\n" "Health:" "$health"
  printf "%-18s %s / %s\n" "Platform/runtime:" "$BOS_PLATFORM" "$([[ -n "$active" ]] && bos_profile_value "$active" runtime || echo none)"
  printf "%-18s %s\n" "PID:" "${pid:-none}"
  printf "%-18s %s\n" "Uptime:" "$uptime"
  printf "%-18s %s\n" "CPU:" "$cpu"
  printf "%-18s %s\n" "Process memory:" "$rss"
...
bos_top_once() {
  ...
  local cpu="0" rss_kb="0" rss_bytes=0 uptime="-" total_bytes model_percent=0 free_percent="-" swap_total=0 swap_used=0 swap_percent=0 gpu=""
  ...
  bos_panel_line "Health: [$health_text]   Profile: ${active:-none}   PID: ${pid:-none}   Uptime: $uptime"
